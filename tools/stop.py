...
def register(mcp) -> None:
    @mcp.tool()
    async def stop(device: str = None) -> dict:
        """
        停止 HomePod 播放
        ...
        """
        logger.info(f"[stop] Called with device='{device}'")
        try:
            from pyatv import connect

            device_name = resolve_device_name(device)
            if not device_name:
                logger.error("[stop] No device configured")
                return {"error": "No device configured"}

            active_stream = get_active_stream(device_name)
            if active_stream:
                await active_stream.atv.remote_control.stop()
                logger.info(f"[stop] Stop sent to active stream on '{device_name}'")
                active_stream.task.cancel()
                try:
                    await close_atv(active_stream.atv)
                finally:
                    clear_active_stream(device_name, active_stream.task)
                return {"success": True, "device": device_name, "action": "stop", "mode": "active_stream"}
            ...
            logger.info(f"[stop] Stop command sent to '{device_name}'")
            await close_atv(atv)

            return {"success": True, "device": device_name, "action": "stop"}
        except Exception as exc:
            logger.error(f"[stop] Error: {exc}")
